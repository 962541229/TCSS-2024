...
def batch_iter(x, y, z, w, batch_size=128):
    data_len = len(x)
    ...
    for i in range(num_batch):
        ...
        yield x_shuffle[start_id:end_id], y_shuffle[start_id:end_id], z_shuffle[start_id:end_id], w_shuffle[start_id:end_id]


def evaluate(config, model, val_x, val_y, val_length, val_index, test=False):
    model.eval()
    loss_total = 0
    predict_all = np.array([], dtype=int)
    labels_all = np.array([], dtype=int)

    batch_val = batch_iter(val_x, val_y, val_length, val_index)
    with torch.no_grad():
        for x_batch, y_batch, length_batch, index_batch in batch_val:
            batch_nodes = torch.unique(x_batch)
            batch_nodes_doc = index_batch
            ...
            # doc
            frontier_Dis_doc = dgl.in_subgraph(config.g_doc_doc_Dis, batch_nodes_doc[0:-1])
            block_Dis_doc = dgl.to_block(frontier_Dis_doc, batch_nodes_doc[0:-1]).to(config.device)

            frontier_Pmi_doc = dgl.in_subgraph(config.g_doc_doc_Pmi, batch_nodes_doc[0:-1])
            block_Pmi_doc = dgl.to_block(frontier_Pmi_doc, batch_nodes_doc[0:-1]).to(config.device)

            frontier_Top_doc = dgl.in_subgraph(config.g_doc_doc_Top, batch_nodes_doc[0:-1])
            block_Top_doc = dgl.to_block(frontier_Top_doc, batch_nodes_doc[0:-1]).to(config.device)
            ...
            outputs = model((block_Dis, block_Pmi, block_Top), (block_Dis_doc, block_Pmi_doc, block_Top_doc), x_batch, length_batch)
...
def train(config, model):
    ...
    doc_index_total = np.arange(len(length_id_total))
    ...
    doc_index_total = torch.LongTensor(doc_index_total)
    ...
    real_train_doc_index, val_doc_index, text_doc_index = doc_index_total[real_train_mask], doc_index_total[val_mask], doc_index_total[test_mask]
    ...
    for e in range(epoch_num):
        ...
        batch_train = batch_iter(real_train_x, real_train_y, real_train_length, real_train_doc_index, batch_size=config.batch_size)
        for x_batch, y_batch, length_batch, doc_index_batch in batch_train:
            model.train()

            # sample L-th neighbour node
            batch_nodes = torch.unique(x_batch)
            batch_nodes_doc = doc_index_batch
            ...
            if total_batch % 1 == 0:
                ...
                dev_acc, dev_loss = evaluate(config, model, val_x, val_y, val_length, val_doc_index)
                test_acc, test_loss = evaluate(config, model, test_x, test_y, test_length, text_doc_index)
