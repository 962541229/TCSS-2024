...
def construct_dis_graph_numpy(config):
    docs, embedding_pretraineds = config.docs, np.array(config.embedding_pretrained_doc, dtype=np.float32)
    ...
    for i in range(len(docs)):
        ...
        indices = top_n_indices(distances, n=config.each_doc_out_degree_dis)

        s_nodes.extend(indices)
        g_nodes.extend([i] * len(indices))

    g = dgl.graph((s_nodes, g_nodes), num_nodes=len(docs))
    g = dgl.add_self_loop(g)
    g = dgl.to_simple(g)

    dgl.save_graphs(config.doc_doc_Dis_dir, g)
