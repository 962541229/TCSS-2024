...
def construct_pmi_graph_doc(config):
    word_documents_adjmatrix_train, y_train, length_id_train = process_dataset_sequence(config, 'train')
    word_documents_adjmatrix_test, y_test, length_id_test = process_dataset_sequence(config, 'test')

    word_documents_adjmatrix_total = np.concatenate((word_documents_adjmatrix_train, word_documents_adjmatrix_test),
                                                    axis=0)
    length_id_total = np.concatenate((length_id_train, length_id_test)).astype(np.int)
    ...
    num_docs = word_documents_adjmatrix_total.shape[0]

    for i in range(num_docs):
        doc_i_words = word_documents_adjmatrix_total[i][:length_id_total[i][0]]
        doc_i_set = set(doc_i_words)
        ...
        for j in range(num_docs):
            ...
            doc_j_words = word_documents_adjmatrix_total[j][:length_id_total[j][0]]
            doc_j_set = set(doc_j_words)

            common_words = doc_i_set.intersection(doc_j_set)

            similarity = 0
            for w1 in common_words:
                for w2 in common_words:
                    if w1 != w2:
                        similarity += pmi_matrix[w1, w2]

            doc_similarities.append((j, similarity))

        top_docs = heapq.nlargest(config.each_doc_out_degree_pmi, doc_similarities, key=lambda x: x[1])

        for doc_j, sim in top_docs:
            s_nodes.append(i)
            d_nodes.append(doc_j)


    g = dgl.graph((s_nodes, d_nodes), num_nodes=num_docs)
    g = dgl.add_self_loop(g)
    g = dgl.to_simple(g)

    dgl.save_graphs(config.doc_doc_Pmi_dir, g)
