...
def construct_topic_graph(config):
    ...
    for topic_id in range(lda_model.num_topics):
        ...
        for doc_id, topics in enumerate(doc_topics):
            for topic, prob in topics:
                if topic == topic_id:
                    topic_probabilities.append((doc_id, prob))

        topic_probabilities.sort(key=lambda x: x[1], reverse=True)
        top_n_docs = topic_probabilities[:config.each_doc_out_degree_top]

        for i in range(len(top_n_docs)):
            for j in range(len(top_n_docs)):
                if i != j:
                    doc1, _ = top_n_docs[i]
                    doc2, _ = top_n_docs[j]
                    s_nodes.append(doc1)
    ...
    g = dgl.graph((s_nodes, d_nodes), num_nodes=word_documents_adjmatrix_total.shape[0])
    g = dgl.add_self_loop(g)
    g = dgl.to_simple(g)

    dgl.save_graphs(config.doc_doc_Topic_dir, g)
    (g,), _ = dgl.load_graphs(config.doc_doc_Topic_dir)
